Option Explicit
'==================================================================
'  シフト自動作成  ＜標準モジュール ShiftAuto v7.1.0＞  2026-08-24
...
'  v7.1.0 変更:
'   ・集計行ラベル(医師数(診)・薬剤師出勤数・過不足 等)を氏名として
'     扱わない IsNonName 判定を追加(範囲がずれても二重に防御)
'   ・フォールバック範囲を B11:AF24(医師数(診)行を含めない)に修正
'   ・設定シート自動生成でも集計行ラベルを取り込まない
'  ※名前付き範囲「シフトパレット範囲」は
'    B10の次行 ? 医師数(診)の1つ上 を指すこと(現行 B11:AF24)
'==================================================================
...
Private Const FB_SHIFT   As String = "B11:AF24"
Private Const FB_PALETTE As String = "B29:N29"
' 予定ステータス
...
Public Sub シフト自動作成()
    ...
    End If
    docRow = CLng(mres)
    '--- 入力範囲が集計行に食い込んでいないか確認 ---
    If grid.Row + mNP - 1 >= docRow Then
        MsgBox "シフト入力範囲(" & grid.Address(False, False) & ")が" & vbCrLf & _
               "「" & DOC_LABEL & "」行(" & docRow & "行)に達しています。" & vbCrLf & vbCrLf & _
               "名前付き範囲「シフトパレット範囲」の終端を" & vbCrLf & _
               "「" & DOC_LABEL & "」の1つ上に修正してください。" & vbCrLf & _
...
Public Sub シフト設定チェック()
    ...
    If Not IsError(mres) Then
        docRow = CLng(mres)
        If grid.Row + grid.Rows.Count - 1 >= docRow Then
            warnRange = "■ 入力範囲が「" & DOC_LABEL & "」行(" & docRow & "行)に達しています" & vbCrLf & _
                        "　名前付き範囲「シフトパレット範囲」の終端を修正してください" & vbCrLf & vbCrLf
        End If
    End If
...
Public Sub シフトログリセット(Optional ByVal ask As Boolean = True)
    ...
    lg.Rows("2:" & lr).Delete
End Sub
② 標準モジュール ShiftClick（パレット入力）
前回お渡しした v8 から、フォールバック定数の修正と見出し位置の修正を反映した版です。

Option Explicit
'==================================================================
'  シフト表 クリック入力マクロ  ＜標準モジュール ShiftClick v8.1＞
'  最終更新: 2026-08-24
'  パレット構成（横1行）:
'    過不足行 +1 = ★マーカー行 / +2 = パレット本体 / +3 = ラベル行
'    （現状: 27行=過不足 / 28行=マーカー / 29行=本体 / 30行=ラベル）
'  動的範囲（ブック定義の名前付き範囲）:
'    「シフトパレット範囲」= シフト入力欄（B10の次行?医師数(診)の1つ上）
'    「シフトパレット」    = パレット本体（過不足行+2、ラベル行の最終セルまで）
'  v8.1: 見出しをマーカー行の左に配置 / フォールバックを現行位置に更新
'==================================================================
'------------------------ 設定 ここから ---------------------------
' 動的範囲の名前
...
Public Const SHIFT_RANGE       As String = "B11:AF24"
