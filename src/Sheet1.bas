Option Explicit
'==================================================================
'  シフト表 クリック入力マクロ + 手動変更ログ + 期替わりリセット
'  ＜シートモジュール＞
'==================================================================
...
' 値が変わった → A1なら期替わりリセット確認 / 入力欄なら「手動」ログ記録
Private Sub Worksheet_Change(ByVal Target As Range)
    ...
    If Not Application.Intersect(Target, Me.Range("A1")) Is Nothing Then
        If IsDate(Me.Range("A1").Value) Then
            If MsgBox("対象月が変わりました。変更ログをリセットしますか?" & vbCrLf & _
                      "(前の期のログはすべて消去されます)", _
...
Private Sub CacheGridState(ByVal rng As Range)
    ...
    If g.Cells.Count > 200 Then Exit Sub   ' 大量選択は記録対象外
