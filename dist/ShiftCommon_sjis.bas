...
Option Explicit
'==================================================================
'  シフト表 共通モジュール ＜標準モジュール ShiftCommon v3.2＞
'  2026-08-28
'
'  v3.2: パレット定義のコメントが直下の IDX_* 定数と食い違っていたのを
'        直した。コメントは「3 切替 / 4 色消 / 17-25 医師名(9名) / 26項目」
'        だが、定数は IDX_UNDO=3 / IDX_EXPORT=4 / IDX_DOC_FIRST=19 で、
'        実際は DOC_SLOTS=10 を含む29項目。動作は定数側で正しく、
'        コメントだけが古かった。同じ並びが ShiftSetup にもある。
'
...
'==================================================================
'  パレットの固定位置(左からの番号)
'  B列から AD列 = 29項目 (18 + DOC_SLOTS(10) + 1)
'    1 OFF   2 自動   3 戻す   4 出力   5 切替   6 色消
'    7 背景緑 8 背景橙 9 背景灰   ← 値は空。背景色のみ持つ
'   10 消去                        ← 値は空
'   11 ○  12 ●  13 ▲
'   14 公休 15 希休 16 夏休 17 有休 18 有休※
'   19-28 医師名(DOC_SLOTS個)
'   29 銀行
'  ※医師名の範囲は位置ではなくラベルで判定する(IsDoctorStamp)。
'    枠を増減しても追随できるよう、末尾の位置は定数に持たない。
'==================================================================
